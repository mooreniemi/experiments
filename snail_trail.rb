...
class Trailer
  ...
  def snail
    ...
    @collection = @collection.each {|a| a.delete_at((@collection.length - 1)) } # delete rightmost column
...
RSpec.describe "snail trail algorithm" do
  ...
  it "takes right corners" do
    ...
    expect(Trailer.new([[1,2],[3,4]]).send(:left_corner)).to eq([1,3,4])
  end
